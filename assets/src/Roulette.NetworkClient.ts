...
class RequestListener {
    ...
    constructor(target: cc.Component, route: string, callback: (res: Object) => void) {
        this.target = target;
        this.route = route;
        this.callback = callback;
    }
...
export default class NetworkClient {
    ...
    public static serverCurrentTimeMillis(): number {
        return Date.now() - this.TIME_DISTANCE + Math.round(NetworkClient.MIN_PING / 2);
    }
    public static systemCurrentTimeMillis(): number {
        return Date.now();
    }

    public isUseWSS: boolean = Configs.Connect.IsUseWSS;
    public isAutoReconnect: boolean = true;

    private ws: WebSocket = null;
    private host: string = Configs.Connect.Host;
    private port: number = Configs.Connect.Port;
    private isForceClose = false;
    private onOpenes: Array<NetworkListener> = [];
    private onCloses: Array<NetworkListener> = [];
    private xorKey = null;//"dmVyeSBzZWNyZXQ";
    ...
    private onMessage(ev: MessageEvent) {
        ...
        if (this.xorKey) data = this.doXOR(data, 0, data.length);
    ...
    private send(msg: any) {
        ...
        if (this.xorKey) source = this.doXOR(source, 0, source.length);
        this.ws.send(source);
    }
    ...
    private doXOR(source: Uint8Array, start: number, count: number) {
        ...
        for (let i = start; i < end; i++) {
            source[i] = source[i] ^ this.xorKey.charCodeAt(index % this.xorKey.length);
            index++;
        }
        return source;
    }
    ...
    public connect() {
        ...
        if (this.ws == null) {
            console.error("start connect: " + this.host + ":" + this.port);
    ...
    public ping(callback: () => void = null, target: cc.Component = null) {
        ...
        this.request("ping", null, (res) => {
            console.log(res["time"], new Date(), Date.now() - t);
            NetworkClient.PING = Date.now() - t;
            if (NetworkClient.MIN_PING < 0 || NetworkClient.PING < NetworkClient.MIN_PING) {
                NetworkClient.MIN_PING = NetworkClient.PING;
                NetworkClient.TIME_DISTANCE = Date.now() - res["time"];
            }
            if (callback != null) callback();
        }, target != null ? target : NetworkClient.NODE_FIXED);
    }
}
